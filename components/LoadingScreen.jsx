    ...
    >
      <style>{`
        .loading-text-anim {
          ...
          font-family: var(--font-theseasons-lt);
          font-size: clamp(8rem, 25vw, 14rem);
          /* Desktop default */
          transform: translate(-50%, calc(-50% + 5rem)) scale(1.8);
        }
        .loading-text-anim.scaling {
          transform: translate(-50%, calc(-50% - 0rem)) scale(1);
        }
        ...
        @media (max-width: 768px) {
          ...
          .loading-text-anim.scaling {
            transform: translate(-50%, calc(-50% + 2.5rem)) scale(1.25);
          }
        }
      `}</style>
      <h1
        className={`absolute top-1/2 left-1/2 font-[550] uppercase text-[#dddddd] loading-text-anim ${animationPhase === 'scaling' ? 'scaling' : ''}`}
      >
        {typedText}
      </h1>
    </div>
